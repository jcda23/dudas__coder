...
const printCar = () => {
  ...
  printFooter();
  localStorage.setItem("car", JSON.stringify(buyCar));
};
...
const printFooter = () => {
  ...
  const nPrecio = Object.values(buyCar).reduce(
  ...
  const emptyCarBtn = document.getElementById("empty__car");
...
const btnSelect = (e) => {
  ...
  if (e.target.classList.contains("btn-info")) {
    const editCarrito = buyCar[e.target.dataset.id];
    editCarrito.cantidad++;
    buyCar[e.target.dataset.id] = { ...editCarrito };
    printCar();
  }
  ...
  if (e.target.classList.contains("btn-danger")) {
    const editCarrito = buyCar[e.target.dataset.id];
    editCarrito.cantidad--;
    if (editCarrito.cantidad === 0) {
      delete buyCar[e.target.dataset.id];
    }
    printCar();
  }
  e.stopPropagation();
...
async function main() {
  ...
  let sesionActive = localStorage.getItem("loginActive");
  if (sesionActive === "true") {
    window.location.href = "../../views/carrito.html";
  }
  if (sesionActive === undefined || sesionActive === null) {
    ventanaModal(
